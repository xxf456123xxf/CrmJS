...
const newEntity = {
  onload() {
    ...
    attr('ownerid').setLookup('systemuser', 'guid', '管理员') //lookup时赋值
    ...
    ent.reload() //重新加载页面
    ent.save() //保存
    ent.disabled(['name']) /*or*/ ent.disabled(name) //禁用
    ent.disabledAll() //禁用全部
    ent.hide(['name']) //隐藏
    ent.show(['name']) //显示
    ent.Tabs(['name'], 'show').Tabs('name', 'required') //显示并必填
